...
async function generateTask(userQuery) {
  try {
    ...
    const prompt = `Convert the following task description into a structured format with Title, Details, and Category.
...
const showResult = async function() {
  const result = await generateTask('Watch superman movie called man of steel');
  console.log('result from generate task AI:', result)
}
